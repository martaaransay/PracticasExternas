...
def calculate_Pc_abundance(path,
                           ...
                           columns_name):
    """
    Reads an Excel file and calculates the cross-tabulated abundance of Pc per specie.

    Args
    ------
    path (str): The file path to the .xlsx file.
    n_header (int): The number of header rows to skip.
    columns_name (list): A list containing the two column names to cross-tabulate.

    Return
    ------
    count_table (pd.DataFrame): A DataFrame containing the cross-tabulated counts of Pc per species.
    """
...
def calculate_Pc_abundance_eskapee(count_table,
                                   eskapee = ["Escherichia coli", "Klebsiella pneumoniae", 
                                              ...
                                              "Enterobacter spp."] ):
    """
    Filters the abundance count table to include only the specified ESKAPEE species, 
    grouping all Enterobacter variants into a single row (Enterobacter spp.).

    Args
    ------
    count_table (pd.DataFrame): The general count DataFrame generated by calculate_Pc_abundance().
    eskapee (list, optional): A list of specific ESKAPEE species names to filter by. 
                              Defaults to standard ESKAPEE pathogens.

    Return
    ------
    eskapee_count_table (pd.DataFrame): A filtered DataFrame containing the Pc counts for ESKAPEE species.
    """
...
def write_count_table_xlsx(count_table,
                           ...
                           out_path):
    """
    Saves the general and ESKAPEE count tables into a .xlsx file across different sheets.

    Args
    ------
    count_table (pd.DataFrame): The general Pc abundance DataFrame.
    eskapee_count_table (pd.DataFrame): The filtered ESKAPEE Pc abundance DataFrame.
    out_path (str): The destination file path where the .xlsx file will be saved.
...
def count_table_plots(count_table,
                      ...
                      eskapee = ["Escherichia coli", "Klebsiella pneumoniae", 
                                              ...
                                              "Enterobacter spp."]):
    """
    Generates and displays two side-by-side bar plots: 
    - one for general relative Pc abundance 
    - one for stacked Pc distribution within ESKAPEE species.
